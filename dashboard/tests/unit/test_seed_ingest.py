...
def test_run_once_if_underfilled_skips_when_db_full(tmp_path: Path) -> None:
    ...
    try:
        ...
        DeepCardRepo(conn).upsert(DeepCard(cap_id="memory.session_checkpoint"))
    finally:
        conn.close()
...
def test_run_does_not_overwrite_existing_when_not_forced(tmp_path: Path) -> None:
    ...
    seed = tmp_path / "seed.jsonl"
    _write_seed_jsonl(seed, ["memory.session_checkpoint"])
    db = tmp_path / "board.db"
    ...
    try:
        DeepCardRepo(conn).upsert(
            DeepCard(cap_id="memory.session_checkpoint", what="user-edited content")
        )
    finally:
        conn.close()
    ...
    conn = open_db(db)
    try:
        card = DeepCardRepo(conn).get("memory.session_checkpoint")
    finally:
        conn.close()
...
def test_run_force_overwrites_existing(tmp_path: Path) -> None:
    ...
    conn = open_db(db)
    try:
        DeepCardRepo(conn).upsert(DeepCard(cap_id="memory.session_checkpoint", what="old content"))
    finally:
        conn.close()
    ...
    try:
        ...
        conn.close()
    assert card is not None
    assert card.what == "what for memory.session_checkpoint"
