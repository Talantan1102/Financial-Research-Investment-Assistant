...
1. **先读真实代码再下笔。** 写代码、评估、PR 描述、绩效自评前，先读真实 diff 和源文件（`git diff` / `git show` / 直接读文件），不靠 PR 摘要或记忆里的假设——多次返工都源于"基于摘要写、没看真实改动"。
2. **没真跑过的不声称跑过。** 特性/修复走 TDD（先写失败测试 → 实现 → 跑活体 → CI 真绿才算完）；测试、workflow、CI 没实际触发过，不说"在跑 / 已通过"。
3. **修问题 / 开 PR 前先比对 main。** 当前分支可能 stale，洞可能 main 上早修好；动手前 `git log origin/main` / 看近期 PR，只修 main 也缺的那部分，别重复劳动。
4. **共享 checkout / 并发 session 先查再推。** 多 session 并发同一仓时，动手或合并前先 `git fetch` 查并发提交；要改/提交的活优先隔离到独立 worktree + 分支（主 checkout 只读），否则会互相取消 CI run、撞 push（#187 撞车即此）。
5. **调研结论先核实关键事实。** 研究/调研类输出先核实型号、版本、实体身份等关键事实再下结论，带来源、讲大白话，不靠猜。

> 第 3、4 条在维护者私人 memory 里有更细的踩坑详版，这里是进 git、跨机/团队可见的精简版。
...
### Agent 模拟交易(2026-07-22 Plan 2 委托撮合 ship)
...
- [Agent 模拟交易 Plan 2 委托撮合 ship 完](docs/claude-context/paper-order-matching-done.md) — 可确认委托/预占/五档部分成交/T+1 与费用原子结算 + Celery 重投幂等/对账/可观测性;Agent 工具、确认卡 UI 和自选股写操作留 Plans 3-4
