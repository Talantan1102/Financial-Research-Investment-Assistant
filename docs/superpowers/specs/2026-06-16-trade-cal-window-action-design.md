...
pass@k 重测里观察到:agent 回答"近一年涨幅/相关性"这类问题时,**反复调 trade_cal 4–6 次**只为把"近一年"落成一段交易日窗口——`is_open(今天)` → `latest` → 区间起点 → 区间终点,一次一问。这不是正确性问题(答案对),是**步数虚高 + 费 token + 链路变长**(将来 RL 功劳分配更难)。
...
加 trade_cal 第 7 个动作 `window`:给"今天 + 一个周期码",**一次**返回解析好的交易日窗口。4 次往返 → 1 次。
...
- live 重跑一道"近一年"题,trace 里日期解析从"trade_cal ×4"降到"window ×1"(adoption 由文档 nudge 引导,允许 agent 仍偶尔多调,但能力已可用);
