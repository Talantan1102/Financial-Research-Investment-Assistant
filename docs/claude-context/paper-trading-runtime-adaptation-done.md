...
- 下单卡可编辑股票代码、名称、方向、数量、订单类型和限价；批准只使用最后一次成功预览返回的完整 canonical 参数。
- 每个用户只有一个默认模拟账户；订单、成交、资金流水和持仓沿用领域服务写入，`Trade` 仍是持仓事实来源。
- Resume 强制绑定不可复用的 `pause_id`；多标签旧卡、连续 pause 和跨 Run 恢复都以数据库一致快照为准。
- 旧数据库通过显式幂等迁移补齐模拟交易/自选股 schema；账户 reset 后，归档 generation 的历史持仓不再进入监控范围。
...
旧库升级测试从 main 旧 `positions`/`trades` schema 和已有数据出发，验证新增表、列、FK、CHECK、唯一约束、partial index 及 watchlist append-only trigger/function 的定义级一致性、幂等、并发锁和失败回滚。Run 恢复测试覆盖四类陈旧响应、HTTP 409 零副作用、多标签恢复、单 PostgreSQL snapshot 下的消息/控制状态一致性，以及 active Run 的 SSE handoff。监控测试覆盖 reset 后旧 generation 排除、手工持仓保留、新 generation 恢复和跨来源去重。
...
未声称运行远端 CI。第二次整分支独立终审结论为 Critical 0、Important 0、Minor 0。完整 `backend/tests/unit/skills` 扩大组合曾运行 140 条，其中 127 passed、2 skipped、11 failed；11 条失败均来自三个 SkillLoader 文件在 Windows 用默认 CP936 写 fixture、生产按 UTF-8 读，以及 CRLF 磁盘大小与解码后内容大小不一致。它们不属于上述 1363 条 paper-trading 定向套件，也未计入上述 106 passed。前端构建仍有既有 `%VITE_TITLE%`、ECharts 分包和大 chunk 警告；后端仍有既有 SQLAlchemy、Pydantic 和 `datetime.utcnow()` 弃用警告。
