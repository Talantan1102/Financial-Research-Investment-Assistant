...
- Windows 不支持 POSIX `resource`/`fork`/`killpg`；SkillExecutor 在 Windows 跳过 rlimit、用 `taskkill /T /F` 回收进程树，POSIX 行为保持不变。
...
Playwright 使用真实 Chromium 和真实 React 页面交互，API 边界按仓库现有 E2E 约定由 `page.route` 提供确定性状态机；它验证 Run 等待审批、编辑预览、批准恢复、会话重载、账户页订单/资金终态，以及自选股无确认增改删。真实 PostgreSQL 领域终态和真实 Celery/Redis Worker 由上述后端集成与 Worker E2E 覆盖。

未声称运行远端 CI。前端构建仍有既有 `%VITE_TITLE%`、ECharts 分包和大 chunk 警告；后端仍有既有 SQLAlchemy、Pydantic 和 `datetime.utcnow()` 弃用警告，均不影响本轮退出码。
