    ...
    def _generate_report(self) -> dict[str, Any]:
        ...
        report = {
            ...
            "failed_tests": [
                r
                for r in self.results
                if r["status"] in ["EXECUTION_FAILED", "EXCEPTION", "TOOL_NOT_FOUND"]
            ],
...
async def main():
    """主函数"""
    # 测试用例文件路径
    test_cases_file = os.path.join(os.path.dirname(__file__), "simple_test_cases.json")
    ...
    report_file = os.path.join(
        os.path.dirname(__file__), f"tool_validation_report_{timestamp}.json"
    )
