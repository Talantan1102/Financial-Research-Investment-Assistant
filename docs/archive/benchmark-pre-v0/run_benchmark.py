...
class GLM5MCPRunner:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise RuntimeError(f"请设置 {self._get_env_var_name()} 环境变量或传入 api_key")
    ...
    async def init_mcp(self):
        ...
        self.mcp_client = MCPClient(
            server_script_path=self.server_path, connect_timeout=30.0, call_timeout=60.0
        )
...
请确保选择正确的 Skill，否则无法获取所需数据。"""
    ...
    def _call_zhipu(self, messages: list[dict], tools: list[dict] | None = None):
        """调用智谱 API"""
        try:
            if tools:
    ...
    def _call_dashscope(self, messages: list[dict], tools: list[dict] | None = None):
        """调用 DashScope API"""
    ...
    async def _execute_skill(self, skill_name: str, tool_name: str, arguments: dict) -> Any:
        """通过 MCP 执行 Skill 工具"""
        try:
            result = await self.mcp_client.call_tool(
                "execute_skill_tool",
                {"skill_name": skill_name, "tool_name": tool_name, "arguments": arguments},
            )
            return result
        except Exception as e:
            return {"error": str(e)}
    ...
    async def _get_skill_md(self, skill_name: str) -> str:
        ...
        try:
            result = await self.mcp_client.call_tool("skill", {"name": skill_name})
    ...
    async def _get_skill_tools(self, skill_name: str) -> list[dict]:
        """获取 Skill 的工具列表"""
        try:
            result = await self.mcp_client.call_tool("get_skill_tools", {"name": skill_name})
            if isinstance(result, dict) and result.get("success"):
    ...
    async def _run_conversation(self, query: str, expected_skill: str) -> dict:
        """
        执行完整对话流程
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "reason": {"type": "string", "description": "选择该 Skill 的原因"},
                        },
        ...
        if not tool_calls:
            ...
            return {"skill": predicted_skill, "tools": [], "tool_calls": []}
        ...
        if not skill_tools:
            print("无工具")
            return {"skill": predicted_skill, "tools": [], "tool_calls": tool_calls_history}
                ...
                msg_dict["tool_calls"].append(
                    {
                        "id": tc.id
                        if hasattr(tc, "id")
                        else f"call_{len(msg_dict['tool_calls']) + 1}",
                        "type": "function",
                        "function": {
                            "name": tc.function.name
                            if hasattr(tc, "function")
        ...
        try:
            ...
            answer = (
                assistant_msg.content if hasattr(assistant_msg, "content") else str(assistant_msg)
            )
            # 添加最终回答到 messages
    ...
    async def run_single_test(self, test_case: dict) -> TestResult:
        ...
        try:
            ...
            # 简化评估：只关心工具调用是否正确
            skill_correct = (
                (predicted_skill == expected_skill)
                if expected_skill
                else (predicted_skill == "" or predicted_skill is None)
            )
            tools_correct = (
                set(predicted_tools) == set(expected_tools)
                if expected_tools
                else (len(predicted_tools) == 0)
            )
            ...
            test_result = TestResult(
                ...
                raw_response=json.dumps(tool_calls_summary, ensure_ascii=False)
    ...
    def _print_result(self, result: TestResult):
        ...
        print(
            f"  [{status}] {mode_tag} {result.test_id}: "
    ...
    def print_summary(self, report: dict[str, Any]):
        ...
        failed_results = [
            ...
            if not r.skill_correct or not r.tools_correct or r.error_occurred
        ]
        if failed_results:
            print(f"\n{'=' * 60}")
            ...
            for r in failed_results:
                ...
                print(f"查询: {r.query}")
                print(
                    f"期望Skill: {r.expected_skill} | 实际: {r.predicted_skill} {'✅' if r.skill_correct else '❌'}"
                )
                print(
                    f"期望Tools: {r.expected_tools} | 实际: {r.predicted_tools} {'✅' if r.tools_correct else '❌'}"
                )
...
async def main():
    ...
    parser.add_argument("--model", default="glm-4-plus", help="模型名称 (默认: glm-4-plus)")
    parser.add_argument(
        "--provider",
        default="auto",
    ...
    parser.add_argument("--test-file", default=None, help="测试用例文件路径")
    ...
    if args.test_file:
        ...
        test_cases_file = os.path.join(
            os.path.dirname(__file__), "testcases", "phase1_single_tool_calls.jsonl"
        )

    # 创建 runner
    runner = GLM5MCPRunner(model=args.model, api_key=args.api_key, provider=args.provider)
    ...
    report_file = os.path.join(
        os.path.dirname(__file__), "reports", f"benchmark_{args.model}_{timestamp}.json"
    )
