...
class SkillDocParser:
    ...
    def parse(self) -> Tuple[Dict[str, Tool], Optional[SkillMetadata]]:
        """解析文档，提取工具和元数据"""
        self.metadata = self._parse_frontmatter()
        self.tools = self._parse_tools()
        return self.tools, self.metadata

    def _parse_frontmatter(self) -> Optional[SkillMetadata]:
        """解析 YAML frontmatter"""
    ...
    def _parse_tools(self) -> Dict[str, Tool]:
        ...
        for match in tool_matches:
            ...
            if '-' in tool_name or tool_name.lower() in ['auto', 'invoked', 'example', 'note', 'warning', 'tip', 'hint']:
                continue
...
class CodeParser:
    ...
    def _parse_tool_block(self, block: str) -> Optional[Tool]:
        """解析单个工具注册块"""
        # 提取工具名
    ...
    def _parse_single_param(self, block: str) -> Optional[Parameter]:
        """解析单个参数定义"""
        name_match = re.search(r'name\s*=\s*["\'](\w+)["\']', block)
        if not name_match:
            return None
    ...
    def _parse_default_value(self, default_str: Optional[str]) -> Any:
        """解析默认值"""
...
class ProgressiveDisclosureChecker:
    ...
    def _check_list_resources(self):
        ...
        if found_skills == expected_skills:
            self.results.append(CheckResult('✅', f'list_resources: 返回 {len(found_skills)} 个 Skill'))
        else:
            ...
            self.results.append(CheckResult('⚠️', f'list_resources: Skill 数量不匹配', details))
    ...
    def _check_read_resource(self):
        ...
        if missing_skills:
            self.results.append(CheckResult('❌', f'read_resource: SKILL.md 缺失', missing_skills))
        else:
...
class SkillConsistencyChecker:
    ...
    def __init__(self, skill_name: str, doc_tools: Dict[str, Tool], code_tools: Dict[str, Tool],
                 doc_metadata: Optional[SkillMetadata], code_tool_count: int):
    ...
    def _check_tool_names(self):
        ...
        if not only_in_doc and not only_in_code:
            self.results.append(CheckResult(
                '✅',
                f'工具名一致: {matched_count}/{matched_count}'
            ))
        else:
            self.results.append(CheckResult(
                '⚠️',
                f'工具名部分一致: {matched_count}/{total}'
            ))

            for name in sorted(only_in_doc):
                self.results.append(CheckResult(
                    '❌',
                    f'工具缺失: 文档中有 {name}, 代码中没有'
                ))

            for name in sorted(only_in_code):
                self.results.append(CheckResult(
                    '❌',
                    f'工具缺失: 代码中有 {name}, 文档中没有'
                ))
        ...
        for tool_name in sorted(common_tools):
            ...
            if not only_in_doc and not only_in_code:
                ...
                if param_issues:
                    result = CheckResult(
                        status='⚠️',
                        message=f'参数不一致: {tool_name}',
                        details=param_issues
                    )
                else:
                    result = CheckResult(
                        status='✅',
                        message=f'参数一致: {tool_name}'
                    )
            else:
                issues = []
                for name in sorted(only_in_doc):
    ...
    def _check_param_details(self, doc_param: Parameter, code_param: Parameter, tool_name: str) -> List[str]:
        """检查单个参数的细节"""
        issues = []

        # 检查类型
        if doc_param.type != code_param.type:
            issues.append(f'  {doc_param.name}: 类型不一致 - 文档: {doc_param.type}, 代码: {code_param.type}')
        ...
        if doc_param.required != code_param.required:
            ...
            issues.append(f'  {doc_param.name}: required 不一致 - 文档: {doc_req}, 代码: {code_req}')
        ...
        if doc_default is not None and code_default is not None:
            if doc_default != code_default:
                issues.append(f'  {doc_param.name}: 默认值不一致 - 文档: {doc_param.default}, 代码: {code_param.default}')
        ...
        elif doc_default is None and code_default is not None:
            if not code_param.required:
...
def check_skill(skill_name: str, base_path: str) -> Tuple[str, List[CheckResult], Optional[SkillMetadata]]:
    """检查单个 skill"""
    ...
    # 执行检查
    checker = SkillConsistencyChecker(skill_name, doc_tools, code_tools, doc_metadata, len(code_tools))
    results = checker.check()

    return skill_name, results, doc_metadata


def print_report(skill_results: Dict[str, List[CheckResult]],
                 progressive_results: List[CheckResult],
                 skill_metadata: Dict[str, Optional[SkillMetadata]],
                 export_json: Optional[str] = None) -> bool:
    """打印检查报告"""
        ...
        for skill_name, results in skill_results.items():
            report["skills"][skill_name] = {
                "metadata": {
                    ...
                    "version": skill_metadata[skill_name].version if skill_metadata[skill_name] else None,
                    "tool_count": skill_metadata[skill_name].tool_count if skill_metadata[skill_name] else 0
                } if skill_metadata[skill_name] else None,
                "checks": [
                    {"status": r.status, "message": r.message, "details": r.details}
                    for r in results
                ]
...
def main():
    ...
    parser.add_argument('--json', '-j', dest='json_output',
                       help='导出 JSON 格式报告到指定文件')
    parser.add_argument('--skill', '-s', dest='skill',
                       help='只检查指定的 skill')
