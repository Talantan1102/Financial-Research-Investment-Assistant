...
def test_real_confirm_dispatch_match_settle_chain_persists_parentage_and_retry_once(
    ...
    with factory() as confirm_session:
        confirmed = _chain_service(confirm_session).confirm(
            user_id=user_id,
            order_id=order_id,
            draft=OrderDraft(
                side=OrderSide.BUY,
        ...
        confirm_span = emit_paper_order_span(
            order_id=order_id,
            name="confirm",
            started_at=CHAIN_NOW,
            attrs={
                "idempotent_replay": False,
                "outcome": "success",
                "status": cast(OrderStatus, confirmed.status).value,
            },
        )
    paper_tasks.dispatch_match_order(order_id, trace_parent_id=confirm_span.span_id)
