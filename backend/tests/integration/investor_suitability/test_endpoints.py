...
def permission_client(db_session: Session) -> Generator[tuple[TestClient, User], None, None]:
    ...
    user = User(
        username=f"permission-{suffix}",
        email=f"permission-{suffix}@test.local",
        hashed_password="x",
    )
    db_session.add(user)
    db_session.flush()
...
def _start(client: TestClient, key: str = "start-star", market: str = "star") -> dict[str, object]:
...
def _qualify(client: TestClient, application_id: str) -> None:
    response = client.put(
        f"/api/v0/market-permissions/applications/{application_id}/profile",
        json={
            "declared_average_assets_20d": "600000.00",
            "securities_experience_months": 36,
            "risk_level": "C4",
        },
    )
    assert response.status_code == 200, response.text
...
def test_user_can_start_submit_confirm_and_list_permissions(
    permission_client: tuple[TestClient, User],
) -> None:
    client, _ = permission_client
    application = _start(client)
    _qualify(client, str(application["application_id"]))
    confirmed = client.post(
        f"/api/v0/market-permissions/applications/{application['application_id']}/confirm",
        json={
            "disclosure_version": "star-risk-disclosure-2026-07",
            "idempotency_key": "confirm-star",
        },
    )
...
def test_cross_user_application_is_not_found(
    permission_client: tuple[TestClient, User], db_session: Session
) -> None:
    client, _ = permission_client
    application = _start(client)
    other = User(
    ...
    response = client.put(
        f"/api/v0/market-permissions/applications/{application['application_id']}/profile",
        json={
            "declared_average_assets_20d": "600000.00",
            "securities_experience_months": 36,
            "risk_level": "C4",
...
def test_stale_disclosure_is_conflict_and_incomplete_profile_is_invalid(
    permission_client: tuple[TestClient, User],
) -> None:
    client, _ = permission_client
    application = _start(client)
...
def test_confirmation_is_idempotent_and_cancel_never_enables(
    permission_client: tuple[TestClient, User],
) -> None:
    client, _ = permission_client
    application = _start(client)
    ...
    cancelled_response = client.post(
        f"/api/v0/market-permissions/applications/{cancelled['application_id']}/cancel"
    )
