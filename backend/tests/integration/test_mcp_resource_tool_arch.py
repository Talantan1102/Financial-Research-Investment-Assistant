    ...
    if data.get("success"):
        ...
        if isinstance(search_data, list) and len(search_data) > 0:
            ...
            if len(search_data) > 0 and isinstance(search_data[0], dict):
                print(
                    f"  第一个结果: {search_data[0].get('name', 'N/A')} ({search_data[0].get('ts_code', 'N/A')})"
                )
    else:
        # 可能因为 API Key 问题失败，但至少验证了路由正确
        error = data.get("error", "")
        print(f"\n[WARN] 工具执行返回错误 (可能是 API 配置问题): {error}")
        print("[PASS] 工具路由和执行链路正确（工具被正确调用）")
...
async def test_call_tool_invalid_format(session):
    ...
    assert "Invalid tool name format" in data.get("error", ""), (
        f"错误信息应包含格式提示: {data.get('error')}"
    )
...
async def test_call_tool_nonexistent_skill(session):
    ...
    assert "not found" in data.get("error", "").lower(), (
        f"错误信息应包含 'not found': {data.get('error')}"
    )
