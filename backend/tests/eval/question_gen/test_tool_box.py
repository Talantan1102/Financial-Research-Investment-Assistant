...
def _box(tmp_path):
    return ToolBox(tushare=_FakeTushare(), skills_root=str(tmp_path / "s"), workdir_root=str(tmp_path / "w"))
...
async def test_exec_data_tool_returns_closes(tmp_path):
    out = await _box(tmp_path).exec(
        "get_stock_daily", {"ts_code": "000938.SZ", "start_date": "20260312", "end_date": "20260612"}
    )
