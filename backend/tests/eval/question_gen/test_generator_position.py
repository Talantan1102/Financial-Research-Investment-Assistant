...
class _StubClose:
    async def get_daily_basic(self, *, ts_code, trade_date=None):
        return pd.DataFrame(
            [{"close": 100.0, "pe": 1.0, "pb": 1.0, "turnover_rate": 1.0, "dv_ratio": 1.0}]
        )


def _run():
    return asyncio.run(
        generator.build_position_cases(_StubClose(), "20260612", lambda t: f"qg-{t}")
    )
...
def test_build_position_cases_count_and_values():
    ...
    pnl = next(
        c for c in cases if c.indicator == "单仓浮盈"
    )  # cost=round(100*0.85,2)=85 → 100*(100-85)=1500
