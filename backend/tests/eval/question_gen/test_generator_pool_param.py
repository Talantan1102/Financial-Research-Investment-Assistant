...
class _StubTushare:
    ...
    async def get_daily_basic(self, *, ts_code, trade_date=None):
        return pd.DataFrame([{"pe": 25.0, "pb": 8.0, "turnover_rate": 1.5, "dv_ratio": 2.0, "close": 100.0}])
    async def get_fina_indicator(self, *, ts_code, end_date=None):
        rows = [{"end_date": "20241231", "roe": 0.25, "debt_to_assets": 0.35, "grossprofit_margin": 0.90, "eps": 5.0, "bps": 35.0}]
...
def test_build_snapshot_cases_with_pool():
    stub = _StubTushare()
    cases = asyncio.run(generator.build_snapshot_cases(stub, "20260612", lambda tag: f"qg-{tag}", pool=_TWO_STOCKS))
