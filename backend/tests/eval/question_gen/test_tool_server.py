...
def test_session_exec_data_then_compute(tmp_path):
    ...
    # 取数
    r1 = c.post(f"/sessions/{sid}/exec", json={"tool": "get_stock_daily",
              "args": {"ts_code": "000938.SZ", "start_date": "20260312", "end_date": "20260612"}}).json()
    assert r1["ok"] and r1["result"]["closes"][-1]["close"] == 28.45
    # 算
    r2 = c.post(f"/sessions/{sid}/exec", json={"tool": "run_python",
              "args": {"code": "result = (28.45/30 - 1) * 100"}}).json()
