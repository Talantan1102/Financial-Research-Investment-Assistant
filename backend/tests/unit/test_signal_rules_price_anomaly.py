...
async def test_green_stable(subject: MonitoringSubject) -> None:
    ...
    r = await PriceAnomalyRule().evaluate(
        subject, _ts(df), MagicMock(), MagicMock(), DEFAULT_THRESHOLDS["price_anomaly"]
    )
    assert r.level == SignalLevel.GREEN


@pytest.mark.asyncio
async def test_yellow_single_day_drop(subject: MonitoringSubject) -> None:
...
@pytest.mark.asyncio
async def test_red_single_day_drop(subject: MonitoringSubject) -> None:
...
@pytest.mark.asyncio
async def test_yellow_60d_cumulative(subject: MonitoringSubject) -> None:
