...
def test_validate_dim_min_tools_violated() -> None:
    ...
    plan = _mk_plan(
        [
            _mk_sub(
                "s1", "财务全景", ["get_financials"]
            ),  # only 1 of {balance,cashflow,financials}
            _mk_sub("s2", "估值", ["get_pe_history"]),
            _mk_sub("s3", "行业地位", ["web_search"]),
            _mk_sub("s4", "风险底线", ["get_holder_change"]),
        ]
    ...
    plan = _mk_plan(
        [
            _mk_sub(
                "s1",
                "财务全景",
                ["get_financials", "get_balance_sheet", "get_cashflow", "get_news"],
            ),
            _mk_sub("s2", "估值", ["get_pe_history"]),
            _mk_sub("s3", "行业地位", ["web_search"]),
            _mk_sub("s4", "风险底线", ["get_holder_change"]),
...
def test_validate_paraphrased_dim_name_fails_coverage() -> None:
    ...
    plan = _mk_plan(
        [
            _mk_sub(
                "s1", "三表分析", ["get_financials", "get_balance_sheet"]
            ),  # paraphrase, no "财务全景"
            _mk_sub("s2", "估值", ["get_pe_history"]),
            _mk_sub("s3", "行业地位", ["web_search"]),
            _mk_sub("s4", "风险底线", ["get_holder_change"]),
