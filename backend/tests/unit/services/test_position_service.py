...
def test_list_for_user_returns_only_active_positions(db_session: Session, user: User) -> None:
    """list_for_user 返回该 user 的全部 Position(包括 quantity=0 的清仓行)。"""
    trade_svc = TradeService(db_session)
    trade_svc.create(
...
def test_update_quote_sets_price_and_timestamp(db_session: Session, user: User) -> None:
...
def test_update_quote_for_unknown_position_raises_lookup(db_session: Session, user: User) -> None:
    pos_svc = PositionService(db_session)
