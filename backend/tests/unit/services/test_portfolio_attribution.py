...
def _holdings():
    return [
        ...
        # 基金:仓位5万、当日-0.2%(不拆板块)
        HoldingDaily(
            ts_code="110011.OF", asset_class="fund_otc", market_value=50000, today_pct=-0.2
        ),
    ]
...
def test_stock_three_layer_closure() -> None:
    ...
    # 三层加总必须严丝合缝等于股票部分贡献
    assert s["market"] + s["sector_excess"] + s["idiosyncratic"] == pytest.approx(
        r.by_class["stock"], abs=1e-9
    )
