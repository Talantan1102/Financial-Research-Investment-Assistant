...
try:
    df_daily = asyncio.run(
        service.generate_daily_data(ts_code="600519.SH", start_date="20240301", end_date="20240310")
    )
    print("✅ daily 成功")
    ...
    if not df_daily.empty:
        ...
        for _, row in preview.iterrows():
            print(
                f"   - {row['trade_date']} | 开:{row['open']:7.2f} | 高:{row['high']:7.2f} | "
...
try:
    df_basic = asyncio.run(service.generate_daily_basic(ts_code="600519.SH", trade_date="20240301"))
    print("✅ daily_basic 成功")
...
print("\n🔍 测试 8: 数据一致性检查...")
try:
    df_check = asyncio.run(
        service.generate_daily_data(ts_code="000001.SZ", start_date="20240301", end_date="20240305")
    )
