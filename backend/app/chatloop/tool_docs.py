...
TOOL_DOCS: dict[str, ToolDoc] = {
    ...
    "memory_search": ToolDoc(
        ...
        doc=(
            ...
            "参数:\n"
            "  query(str,必填)—— 检索查询串。\n"
            "  scope(str,可选,枚举 index|archival|recall|graph,默认 archival)—— "
            "index 只读 MEMORY.md 等价索引 / archival 长期事实库 / recall 近期对话回忆 / "
            "graph 关系图谱。先看索引类别,需要正文时再按需读取详情。\n"
    ...
    "get_paper_account": ToolDoc(
        "get_paper_account", "deferred", "读取当前用户的默认模拟账户。",
        "只读当前用户模拟账户余额，不会触发交易。参数为空。", {},
    ),
    "list_paper_orders": ToolDoc(
        "list_paper_orders", "deferred", "读取当前用户的模拟订单列表。",
        "只读工具。可选 status、ts_code、limit；不用于创建或修改订单。", {},
    ),
    "get_paper_order": ToolDoc(
        "get_paper_order", "deferred", "读取当前用户的一笔模拟订单。",
        "只读工具。参数 order_id(UUID)，跨用户订单不可见。", {"order_id": "string"},
    ),
    "place_paper_order": ToolDoc(
        "place_paper_order", "deferred", "按用户明确要求提交模拟买入或卖出。",
        "仅当用户明确给出股票、方向和数量时使用；研究行情不得调用。执行前暂停并让用户确认，可编辑数量和限价。",
        {"side": "string", "ts_code": "string", "name": "string", "quantity": "integer", "order_type": "string"},
    ),
    "cancel_paper_order": ToolDoc(
        "cancel_paper_order", "deferred", "按用户明确要求撤销模拟订单。",
        "仅在用户明确要求撤销具体订单时使用，执行前必须确认。", {"order_id": "string"},
    ),
    "reset_paper_account": ToolDoc(
        "reset_paper_account", "deferred", "按用户明确要求重置模拟账户。",
        "重置会归档当前代账户，执行前必须确认。参数 initial_cash。", {"initial_cash": "number"},
    ),
    "manage_watchlist": ToolDoc(
        "manage_watchlist", "deferred", "直接增改删当前用户自选股。",
