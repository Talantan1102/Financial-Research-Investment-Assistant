...
class ResearchRequest(BaseModel):
    ...
    query: str
    session_id: str | None = None  # 会话 ID（用于检查点保存）
    max_iterations: int | None = 3
    kb_name: str | None = None  # 本地知识库名称
    search_web: bool | None = None  # 是否搜索网络 (兼容旧版)
    search_local: bool | None = None  # 是否搜索本地知识库 (兼容旧版)
    search_modes: list | None = None  # 搜索模式: ['web', 'local'] (新版)
    version: Literal["v1", "v2"] | None = "v2"  # 版本选择 (v2: 多智能体架构，推荐)
...
async def stream_research_get(
    query: str = Query(
        ..., description="研究问题", example="中国安责险的市场现状和未来发展趋势是什么？"
    ),
    max_iterations: int = Query(3, description="最大迭代次数", ge=1, le=5),
    kb_name: str | None = Query(None, description="本地知识库名称"),
...
async def list_checkpoints(
    status: str | None = Query(None, description="过滤状态: running/paused/completed/failed"),
...
async def resume_research(session_id: str):
    ...
    try:
        ...
        if not info:
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail="No checkpoint found for this session"
            )
