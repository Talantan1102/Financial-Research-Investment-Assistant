...
def get_document_service():
    ...
    return DocumentService(base_url=config["base_url"], api_key=config["api_key"]), config[
        "default_dataset_id"
    ]
...
async def upload_document(
    ...
    try:
        ...
        processing_result = process_document_with_docmind(
            file_path=temp_file_path, file_name=file.filename, index_name=index_name, chunk_size=500
        )
...
async def get_document_list(
    ...
    keywords: str | None = Query(None, description="Keywords to filter document titles"),
    document_id: str | None = Query(None, description="Filter by specific document ID"),
    ...
    try:
        ...
        if response.get("code") != 0:
            ...
            # 其他错误正常抛出异常
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail=f"Failed to retrieve documents: {response}"
            )
...
async def delete_documents(
    ...
    try:
        ...
        if response.get("code") != 0:
            raise HTTPException(
                status_code=HTTP_400_BAD_REQUEST, detail=f"Failed to delete documents: {response}"
            )
        ...
        raise HTTPException(
            status_code=HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Error deleting documents: {str(e)}"
        )


@router.post("/retrieve", status_code=HTTP_200_OK)
async def retrieve_documents(
    request: RetrieveDocumentsRequest, service_info: tuple = Depends(get_document_service)
):
    """
    ...
    try:
        ...
        response = doc_service.retrieve_documents(
            question=request.question, dataset_ids=[dataset_id], document_ids=request.document_ids
        )
