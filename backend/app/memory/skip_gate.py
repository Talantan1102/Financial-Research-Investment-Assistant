...
_STRATEGY_KEYWORDS: frozenset[str] = frozenset(
    {
        ...
        "止损",
        "止盈",
        # 2026-06-05 对话流评估冒烟发现:真实散户口语(看多/转中性/建仓/割肉…)
        # 全部不在书面语白名单里,有效 session 被静默跳过(no_relevant_keyword)。
        # 以下口语/黑话词源自 .claude/skills/retail-investor-voice 语料词典。
        # 取舍:门是成本优化,多放行(多几次 LLM 调用)远比漏抽取(记忆静默丢失)便宜。
        "看多",
        "中性",
        "观点",
        "建仓",
        ...
        "补仓",
        "梭哈",
        # "吃面"不收:与字面吃面歧义太强(test_no_keyword_skipped 的闲聊样例即现实碰撞);
        # 黑话全称"关灯吃面"单独收,歧义消失
        "关灯吃面",
        "站岗",
        "韭菜",
