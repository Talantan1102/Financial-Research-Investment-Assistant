...
class SkillSelection:
    ...
    reasoning: str
    direct_response: Optional[str] = None  # 如果不需要调用工具，直接返回回复
...
class ToolExecutionPlan:
    ...
    control_flow: Optional[ControlFlowDefinition] = None
    reasoning: str = ""
...
class InteractionSession:
    ...
    round_1_result: Optional[SkillSelection] = None
    round_2_result: Optional[ExecutionResult] = None
...
class InteractionEngine:
    ...
    def set_round_1_result(
        ...
        logger.info(f"Round 1 completed: session_id={session_id}, selected_skills={selected_skills}")
    ...
    async def execute_round_2(
        ...
        try:
            ...
            logger.info(f"Round 2 completed: session_id={session_id}, success={execution_result.success}")
    ...
    def get_data_for_round_3(self, session_id: str) -> Dict[str, Any]:
        ...
        execution_summary = {
            ...
            "success_count": len([r for r in session.round_2_result.results.get("tool_results", [])
        ...
        return {
            ...
            "errors": [e.to_dict() for e in session.error_report.errors] if session.error_report else [],
        ...
        return [
            {
                ...
                "user_message": s.user_message[:50] + "..." if len(s.user_message) > 50 else s.user_message,
                "round_completed": sum([
...
def create_parallel_tool_plan(
    ...
    """
    tool_calls = [
        ToolCall(skill=s, tool=t, arguments=a, call_id=c)
        for s, t, a, c in calls
    ]
...
async def example_interaction_flow():
    """示例交互流程"""
    from app.mcp_server.skills import MarketDataSkill, FinancialAnalysisSkill
