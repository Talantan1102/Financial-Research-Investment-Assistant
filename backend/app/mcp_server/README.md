...
金融研投助手 MCP Server 实现了系统编排的两轮交互流程，支持6种控制流模式，提供透明的错误处理机制。

**系统编排架构说明**：
- **对 LLM 暴露两轮**: Round 1 (Skill选择) → Round 2 (工具调用)
- **系统内部处理**:
  1. LLM 选择 skill 后，系统自动调用 `get_skill_tools` 获取工具列表
  2. LLM 调用具体工具 (如 `market_data.get_quote`) 后，系统自动转换为 `execute_skill_tool` 执行
...
┌────────────────────┐      ┌──────────────────────────────┐
│ 系统内部自动调用:     │      │                              │
│ get_skill_tools()  │      │   LLM 选择要调用的具体工具      │
└─────────┬──────────┘      └──────────────┬───────────────┘
          │                                │
          ▼                                ▼
┌────────────────────┐      ┌──────────────────────────────┐
│ 返回工具列表        │ ──→ │  LLM 调用具体工具             │
│                    │      │  (如 market_data.get_quote)   │
└─────────┬──────────┘      └──────────────┬───────────────┘
          │                                │
          │  ┌─────────────────────────────┘
          │  │
