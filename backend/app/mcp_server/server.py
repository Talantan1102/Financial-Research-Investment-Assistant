...
from app.mcp_server.skills.base import BaseSkill, ToolResult
...
from app.mcp_server.control_flow.engine import ControlFlowEngine, ControlFlowContext, ControlFlowType
from app.mcp_server.error_handler import ErrorHandler, ErrorInfo, get_error_handler
...
class FinancialResearchMCPServer:
    ...
    def __init__(self):
        ...
        self.control_flow_engine: Optional[ControlFlowEngine] = None
    ...
    async def _execute_skill_tool(self, skill_name: str, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """执行Skill工具"""
...
"""
    ...
    def get_tool_execution_prompt(
        ...
        target_stock: Optional[Dict[str, str]] = None
    ) -> str:
        """生成工具执行的Prompt"""
        tools_info = json.dumps(self.get_tools_for_skills(selected_skills), ensure_ascii=False, indent=2)
...
"""
    ...
    async def execute_tools(
        ...
        if execution_mode == "parallel":
            results = await self.control_flow_engine.executors[ControlFlowType.SEQUENTIAL].execute_parallel(calls)
        else:
            results = await self.control_flow_engine.executors[ControlFlowType.SEQUENTIAL].execute_sequential(calls)
    ...
    async def execute_control_flow(
        self,
        flow_type: str,
        config: Dict[str, Any]
    ) -> Dict[str, Any]:
...
"""
    ...
    def handle_error(self, error: Exception, context: Dict[str, Any] = None) -> Dict[str, Any]:
        """处理错误"""
        error_info = self.error_handler.handle_error(error, context.get("tool") if context else None)
    ...
    async def analyze_stock(self, symbol: str, analysis_type: str = "comprehensive") -> Dict[str, Any]:
        ...
        if analysis_type in ["comprehensive", "market"]:
            tool_calls.extend([
                {"skill": "market_data", "tool": "get_quote", "arguments": {"symbol": symbol}, "call_id": "quote"},
                {"skill": "market_data", "tool": "get_daily_basic", "arguments": {"symbol": symbol}, "call_id": "valuation"},
            ])

        if analysis_type in ["comprehensive", "financial"]:
            tool_calls.extend([
                {"skill": "financial_analysis", "tool": "calculate_financial_ratios", "arguments": {"symbol": symbol}, "call_id": "ratios"},
            ])

        if analysis_type == "comprehensive":
            tool_calls.extend([
                {"skill": "risk_assessment", "tool": "assess_stock_risk", "arguments": {"symbol": symbol}, "call_id": "risk"},
            ])
...
# 全局Server实例
_server_instance: Optional[FinancialResearchMCPServer] = None
...
if MCP_SDK_AVAILABLE:
    ...
    _mcp_server_instance: Optional[FinancialResearchMCPServer] = None
    ...
    async def get_skill_tools(name: str) -> Dict[str, Any]:
        ...
        return {
            "success": True,
            "skill": name,
            "tools": tools.get(name, {}).get("tools", [])
        }
