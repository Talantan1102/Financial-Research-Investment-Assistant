...
class ConditionEvaluator:
    ...
    @staticmethod
    def _parse_value(value_str: str) -> int | float | str | bool:
        """解析值字符串"""
...
class ControlFlowDefinition:
    ...
    else_branch: list[ToolCall] | None = None

    # SWITCH
    switch_variable: str | None = None
    cases: dict[str, list[ToolCall]] | None = None
    default_case: list[ToolCall] | None = None
...
class ControlFlowExecutor:
    ...
    async def execute(
        self, flow: ControlFlowDefinition, context: ExecutionContext
    ) -> dict[str, Any]:
        """
        执行控制流
    ...
    async def _execute_for_each(
        ...
        for idx, item in enumerate(flow.items):
            ...
            iteration_results.append({"iteration": idx, "item": item, "results": batch_results})
    ...
    async def _execute_if_else(
        ...
        context.log("if_else_evaluation", {"condition": flow.if_condition, "result": condition_met})
    ...
    async def _execute_switch(
        ...
        context.log("switch_evaluation", {"variable": flow.switch_variable, "value": switch_value})
    ...
    async def _execute_parallel(
        self, calls: list[ToolCall], context: ExecutionContext, max_concurrent: int
    ) -> list[dict[str, Any]]:
        """并行执行工具调用"""
    ...
    async def _execute_tool_call(self, call: ToolCall, context: ExecutionContext) -> dict[str, Any]:
        """执行单个工具调用"""
...
def create_sequential_flow(
    tool_calls: list[ToolCall], flow_id: str = "sequential_flow", parallel: bool = True
) -> ControlFlowDefinition:
    """创建顺序执行控制流"""
    return ControlFlowDefinition(
        type=ControlFlowType.SEQUENTIAL, id=flow_id, tool_calls=tool_calls, parallel=parallel
    )
...
def create_filter_flow(
    filter_condition: str, tool_calls: list[ToolCall], flow_id: str = "filter_flow"
) -> ControlFlowDefinition:
    """创建 FILTER 控制流"""
    return ControlFlowDefinition(
