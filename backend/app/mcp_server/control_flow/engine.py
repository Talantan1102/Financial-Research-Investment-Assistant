...
class ControlFlowExecutor(ABC):
    ...
    @abstractmethod
    async def execute(self, context: ControlFlowContext, config: Dict[str, Any]) -> List[ExecutionResult]:
        """执行控制流"""
        pass
    ...
    async def execute_parallel(self, calls: List[ToolCall], max_concurrent: int = 10) -> List[ExecutionResult]:
        """并行执行多个工具调用"""
...
class ForEachExecutor(ControlFlowExecutor):
    ...
    def _substitute_variables(self, template: Dict[str, Any], item: Any, item_var: str, index: int) -> Dict[str, Any]:
        """替换模板中的变量"""
        import json
...
class WhileExecutor(ControlFlowExecutor):
    ...
    def _prepare_iteration(self, template: Dict[str, Any], context: ControlFlowContext, iteration: int) -> Dict[str, Any]:
        """准备单次迭代的调用配置"""
        import copy
...
class ControlFlowEngine:
    ...
    async def execute(
        self,
        flow_type: Union[str, ControlFlowType],
        config: Dict[str, Any],
        context: Optional[ControlFlowContext] = None
    ) -> Dict[str, Any]:
        """
        执行控制流
