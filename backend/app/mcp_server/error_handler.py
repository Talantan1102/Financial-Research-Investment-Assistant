...
class ErrorHandler:
    ...
    def _generate_user_options(self, error: ErrorInfo) -> list[UserOption]:
        ...
        if error.error_type == ErrorType.RATE_LIMIT_ERROR:
            options.append(
                UserOption(action="retry", description="等待60秒后重试", recommended=True)
            )
            options.append(
                UserOption(
                    action="continue",
            ...
            options.append(UserOption(action="abort", description="终止当前查询，稍后再试"))

        elif error.error_type == ErrorType.DATA_NOT_AVAILABLE:
            options.append(
                UserOption(
                    action="skip", description="跳过此数据项，继续分析其他数据", recommended=True
                )
            )
            options.append(UserOption(action="alternative", description="使用替代数据源"))
            options.append(UserOption(action="abort", description="终止当前查询"))

        elif error.error_type == ErrorType.VALIDATION_ERROR:
            options.append(
                UserOption(action="correct", description="修正输入参数后重试", recommended=True)
            )
            options.append(UserOption(action="help", description="查看参数帮助信息"))

        elif error.error_type == ErrorType.NETWORK_ERROR:
            options.append(UserOption(action="retry", description="立即重试", recommended=True))
            options.append(
                UserOption(
                    action="continue", description="基于缓存数据继续", warning="数据可能不是最新的"
                )
            )
            options.append(UserOption(action="abort", description="终止当前查询，检查网络后重试"))

        elif error.error_type == ErrorType.AUTH_ERROR:
            options.append(
                UserOption(action="config", description="检查API密钥配置", recommended=True)
            )
            options.append(UserOption(action="abort", description="终止当前查询"))

        elif error.error_type == ErrorType.CONFIG_ERROR:
            options.append(
                UserOption(action="config", description="检查系统配置", recommended=True)
            )
            options.append(UserOption(action="help", description="查看配置帮助"))

        else:
            # 默认选项
            options.append(UserOption(action="retry", description="重试", recommended=True))
            options.append(UserOption(action="abort", description="终止当前查询"))
            options.append(
                UserOption(action="continue", description="继续（有风险）", warning="可能遇到问题")
            )
    ...
    def should_stop_execution(self, error: ErrorInfo) -> bool:
        ...
        # 配置错误停止
        return error.error_type == ErrorType.CONFIG_ERROR
    ...
    def can_continue_with_partial_data(self, errors: list[ErrorInfo]) -> tuple[bool, str]:
        ...
        data_errors = [
            e
            for e in errors
            if e.error_type in [ErrorType.DATA_NOT_AVAILABLE, ErrorType.DATA_INCOMPLETE]
        ]
...
def handle_exception(
    exception: Exception, tool: str = None, skill: str = None, **context
) -> ErrorReport:
    """
    便捷函数：处理异常
...
def format_error_for_user(report: ErrorReport) -> str:
    ...
    for error in report.errors:
        ...
        lines.append(
            f"- **工具**: {error.skill}.{error.tool}" if error.skill and error.tool else ""
        )
