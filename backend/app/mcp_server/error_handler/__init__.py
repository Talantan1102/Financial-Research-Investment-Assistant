...
class ErrorHandler:
    ...
    def handle_error(
        self, error: Exception, tool: str | None = None, context: dict[str, Any] | None = None
    ) -> ErrorInfo:
        """
        处理错误
    ...
    def _classify_error(self, error: Exception, tool: str | None) -> ErrorInfo:
        """分类错误"""
    ...
    def should_stop_execution(self, error_info: ErrorInfo) -> bool:
        ...
        return len(recent_errors) >= self.max_errors_before_stop
    ...
    def generate_user_options(self, error_info: ErrorInfo) -> list[UserOption]:
        ...
        if error_info.retry_after_seconds:
            ...
            options.append(UserOption(action="retry", description="立即重试", recommended=False))
        ...
        if error_info.error_type == ErrorType.VALIDATION_ERROR:
            options.append(
                UserOption(
                    action="correct_input", description="修正输入参数后重试", recommended=True
                )
            )
    ...
    def format_partial_failure_response(
        self, available_data: dict[str, list[str]], missing_data: list[str], errors: list[ErrorInfo]
    ) -> dict[str, Any]:
        """格式化部分失败响应"""
        return {
            ...
            "user_options": [
                {"action": "retry", "description": "重试获取失败的数据", "recommended": True},
                {
                    "action": "continue",
                    "description": "基于现有数据继续(有风险)",
...
def handle_error(error: Exception, tool: str | None = None) -> dict[str, Any]:
    """便捷函数：处理错误并返回响应"""
