...
class MessageCreate(BaseModel):
    ...
    references_data: dict[str, Any] | None = Field(None, description="引用文档")
    image_results: list[dict[str, Any]] | None = Field(None, description="图片结果")
...
class ChatRequest(BaseModel):
    ...
    session_id: str | None = Field(None, description="会话ID，用于记录对话历史")
...
class RetrievedDocument(BaseModel):
    ...
    link: str | None = Field(None, description="如果是网页，则为链接地址")
...
class ChatWithAttachmentsRequest(BaseModel):
    ...
    question: str = Field(..., description="用户问题")
    attachment_ids: list[str] | None = Field(None, description="附件ID列表")
    search_knowledge: bool = Field(True, description="是否搜索知识库")
    search_web: bool = Field(True, description="是否搜索网络")
