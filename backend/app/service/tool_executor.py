...
class ToolExecutor:
    ...
    async def execute_web_search(self, params: dict[str, Any], context: ReActContext) -> list[dict]:
        ...
        if not query:
            logging.warning("web_search called with empty query, using context query as fallback")
            # 使用用户原始问题作为搜索关键词
            query = context.query
            if not query:
    ...
    def _websearch_sync(self, query: str, count: int = 5) -> list[dict]:
        ...
        headers = {"Authorization": self.search_api_key, "Content-Type": "application/json"}
    ...
    async def execute_data_analyzer(
        ...
        if not data:
            return {"success": False, "error": "没有可分析的数据", "insights": [], "statistics": {}}
    ...
    async def execute_stock_query(
        ...
        try:
            ...
            if stock_code:
                ...
                return {"success": False, "error": "请提供股票代码或关键词", "data": None}
    ...
    async def execute_bidding_search(
        ...
        try:
            ...
            return {"success": False, "error": "招投标查询服务尚未配置", "results": [], "total": 0}
        except Exception as e:
