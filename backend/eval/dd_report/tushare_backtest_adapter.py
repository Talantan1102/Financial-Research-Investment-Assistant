...
class TushareBacktestAdapter:
    ...
    def fetch_balancesheet(self, ts_code: str, **extra: Any) -> list[dict[str, Any]]:
        """fetch 资产负债表, 自动加 end_date <= cut_off + 二次 ann_date 过滤."""
    ...
    def fetch_cashflow(self, ts_code: str, **extra: Any) -> list[dict[str, Any]]:
        """fetch 现金流量表, 自动加 end_date <= cut_off + 二次 ann_date 过滤."""
    ...
    def fetch_daily_kline(
        ...
        """fetch 日 K, end_date 由 cut_off 限定 + 二次 trade_date 过滤.

        缺 trade_date 字段的行被丢("99999999" 哨兵语义,见 _filter_by_ann_date)。
        """
    ...
    def fetch_announcements(self, ts_code: str, **extra: Any) -> list[dict[str, Any]]:
        """fetch 公告, 自动加 end_date <= cut_off + 二次 ann_date 过滤."""
    ...
    def _filter_by_ann_date(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        契约: rows 必须有 ann_date 字段。缺字段的行被 "99999999" 哨兵视为"远
        未来"丢掉 — backtest 下静默 leak 比 crash 更危险, 所以选 silent drop
        而非 raise。test_adapter_drops_rows_with_missing_ann_date 显式守护契约。
        """
