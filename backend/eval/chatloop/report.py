...
def format_scorecard(
    ...
    *,
    ...
    disc_cov = sum(s.disclaimer_present for s in scores)
    ...
    lines = [
        ...
        f"- **免责覆盖率**(应=100%):{_rate(disc_cov, n)}",
    ...
    if failed:
        ...
        lines.append("")
    bad_disc = [s for s in scores if not s.disclaimer_present]
    if bad_disc:
        lines.append("## 免责缺失明细")
        for s in bad_disc:
