...
- 成分股取 index_weight(000906.SH, as_of 当月最近交易日)
- 补 name + sector via stock_basic (industry 列)
- 过滤: ST/*ST、上市不满3年
- 返回清洗后 list[Stock]
"""
...
async def load_csi800(tushare: Any, as_of: str) -> list[Stock]:
    ...
    iw_df = await tushare.get_index_weight(index_code=_CSI800_CODE, trade_date=as_of)
    if iw_df.empty or "con_code" not in iw_df.columns:
        return []

    con_codes = iw_df["con_code"].dropna().unique().tolist()

    # 2. 对每只成分股取 name + industry
    stocks: list[Stock] = []
    for ts_code in con_codes:
        try:
            basic_df = await tushare.get_stock_basic(ts_code=ts_code)
        except Exception:
            continue
        if basic_df.empty:
            continue
