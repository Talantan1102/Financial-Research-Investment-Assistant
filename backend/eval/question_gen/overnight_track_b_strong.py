...
from eval.question_gen import case as case_mod, runner
...
async def main() -> None:
    ...
    ap.add_argument("--cny-per-rollout", type=float, required=True,
    ...
    print(f"[TrackB] 教师={args.model} k={args.k} | 预算¥{args.budget_cny} ÷ "
          f"(k×¥{args.cny_per_rollout}/rollout) → 最多 {max_cases} 题 "
          f"(全集 {len(cases)}) → {len(shards)} 片 | out={args.out}", flush=True)
    ...
    for idx, shard in enumerate(shards):
        ...
        for attempt in (1, 2):
            ...
            try:
                await runner.run_passk(shard, k=args.k, model=args.model,
                                       concurrency=args.concurrency, collect_dir=sdir)
                rows = [json.loads(l) for l in open(traj_f)]
                ...
                print(f"[TrackB] 片{idx:02d} ✅ {len(rows)}轨迹 halt={dict(halt)} "
                      f"干净={clean} {dt:.0f}s | 累计干净={clean_total}", flush=True)
                break
            except Exception as e:  # noqa: BLE001
                print(f"[TrackB] 片{idx:02d} 第{attempt}次失败: {type(e).__name__}: {e}", flush=True)
                if attempt == 2:
