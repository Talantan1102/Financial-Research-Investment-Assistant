...
os.environ['DASHSCOPE_API_KEY'] = os.getenv('DASHSCOPE_API_KEY', 'sk-f5ad885285a44c339968325bdebc5658')
...
try:
    ...
    if not df.empty:
        row = df.iloc[0]
        print(f"   📊 PE:{row.get('pe', 'N/A')} PB:{row.get('pb', 'N/A')} 市值:{row.get('total_mv', 'N/A')}")
except Exception as e:
