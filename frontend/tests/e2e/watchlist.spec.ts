...
test('watchlist CRUD is direct and monitoring switch is editable', async ({ page, context }) => {
  ...
  await expect(page.getByDisplayValue('浦发银行')).toBeVisible()
  await page.getByRole('switch').click()
  await page.getByPlaceholder('股票代码').fill('000001.SZ'); await page.getByPlaceholder('股票名称').fill('平安银行'); await page.getByRole('button', { name: '添加' }).click()
  await expect(page.getByDisplayValue('平安银行')).toBeVisible()
