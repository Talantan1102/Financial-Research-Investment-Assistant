...
async function stubPaperTrading(page: import('@playwright/test').Page) {
  await page.route('**/api/v0/paper-trading/**', async (route) => {
    ...
    const body = url.pathname.endsWith('/account') ? account : url.pathname.endsWith('/holdings') ? [holding] : []
...
test.describe('paper trading account', () => {
  test('shows balances and separates total from sellable holdings', async ({ page, context }) => {
    await seedAuth(context); await stubPaperTrading(page); await page.goto('/paper-trading')
