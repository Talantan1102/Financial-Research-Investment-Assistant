#!/usr/bin/env bash
# Launch GRPO training on 4x A100 80GB
# Actor: FSDP on 2 GPUs | Rollout: sglang on 2 GPUs (tp=2, ep=2)
set -euo pipefail

cd "$(dirname "$0")/../.."

CONFIG="configs/training/grpo_qwen3_30b_4xa100.yaml"

# Optional overrides via env
export SGLANG_TP_SIZE="${SGLANG_TP_SIZE:-2}"
export SGLANG_EP_SIZE="${SGLANG_EP_SIZE:-2}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}"

echo "=============================================="
echo "Launching GRPO on 4x A100"
echo "Config: $CONFIG"
...
# Optional: start sglang server if not already running
